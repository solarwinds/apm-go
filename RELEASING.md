...
## Standard release workflow
 ...
 4. Once the new apm-go release is tested, run the `Lambda Update` workflow. It automatically opens a PR updating `go.mod`/`go.sum` for the Lambda instrumentation and examples to the new version. Review and merge that PR.
 5. Run the `release` workflow manually with the `swolambda_version` input (e.g. `v1.3.7`). It tags and pushes the swolambda module tag; no release is created for this step.
...
The `release` workflow's `swolambda_version` input handles this: it tags the release with the module path (`instrumentation/github.com/aws/aws-lambda-go/swolambda/vX.X.X`) and pushes the tag. Keep the swolambda version number aligned with the APM version.
