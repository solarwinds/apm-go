...
func TestConfiguredHostname(t *testing.T) {
	...
	var old string
	var has bool
	old, has = os.LookupEnv("SWO_HOSTNAME_ALIAS")
	os.Setenv("SWO_HOSTNAME_ALIAS", "testAlias")
	...
	assert.True(t, strings.Contains(buf.String(), "SWO_HOSTNAME_ALIAS"), buf.String())

	if has {
		os.Setenv("SWO_HOSTNAME_ALIAS", old)
	}
}
