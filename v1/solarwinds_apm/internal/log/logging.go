...
func logIt(level LogLevel, msg string, args []interface{}) {
	...
	if level == DEBUG {
		...
		if ok {
			pre = fmt.Sprintf("%-5s [solarwinds_apm] %s:%d ", LevelStr[level], filepath.Base(file), line)
		} else {
			pre = fmt.Sprintf("%-5s [solarwinds_apm] %s:%s ", LevelStr[level], "na", "na")
		}
	} else { // avoid expensive reflections in production
		pre = fmt.Sprintf("%-5s [solarwinds_apm] ", LevelStr[level])
	}
