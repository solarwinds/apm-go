...
func assertInitMessage(t *testing.T, bufs [][]byte) {
	...
	g.AssertGraph(t, bufs, 1, g.AssertNodeMap{
		{"go", "single"}: {Edges: g.Edges{}, Callback: func(n g.Node) {
			...
			assert.True(t, strings.HasSuffix(n.Map["Go.InstallDirectory"].(string), "solarwinds-apm-go/v1/solarwinds_apm"))
...
func TestSampleFlags(t *testing.T) {
	...
	decision := shouldTraceRequestWithURL(testLayer, false, "http://test.com/user123", ModeTriggerTraceNotPresent, nil)
