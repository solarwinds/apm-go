...
func TestOboeEvent(t *testing.T) {
	...
	assert.Error(t, oboeEventInit(nil, nil, RandomOpID))      // init nil evt, md
	assert.Error(t, oboeEventInit(evt, nil, RandomOpID))      // init evt, nil md
	assert.Error(t, oboeEventInit(evt, &emptyMd, RandomOpID)) // init evt, uninit'd md
	md.Init()                                                 // init valid md
	assert.NoError(t, md.SetRandom())                         // make random md
	t.Logf("TestEvent md: %v", md.String())
	assert.NoError(t, oboeEventInit(evt, &md, RandomOpID))     // init valid evt, md
