...
func TestEntryEventNoParent(t *testing.T) {
	now := time.Now()
	e, err := CreateEntry(validSpanContext, now, invalidSpanContext)
	require.Nil(t, err)
	require.NotNil(t, e)
...
func TestEntryEventWithParent(t *testing.T) {
	...
	e, err := CreateEntry(validSpanContext, now, parent)
	require.Nil(t, err)
	require.NotNil(t, e)
...
func TestExitEvent(t *testing.T) {
	now := time.Now()
	e, err := CreateExit(validSpanContext, now)
	require.Nil(t, err)
	require.NotNil(t, e)
...
func TestEventXTraceAndSwTraceCtx(t *testing.T) {
	...
	e, err := CreateEntry(sc, time.Now(), invalidSpanContext)
	require.Nil(t, err)
	require.NotNil(t, e)
	evt, ok := e.(*event)
	require.True(t, ok)
